...
@MainActor public struct AddEditReadingView: View {
    ...
    public var body: some View {
        Form {
            ...
            Section("测量信息") { DatePicker("测量时间", selection: $date, displayedComponents: [.date, .hourAndMinute]); Picker("手臂", selection: $arm) { Text(BPText.localized("未设置", language: language)).tag(BPArm?.none); ForEach(BPArm.allCases, id: \.self) { Text($0.displayName(language: language)).tag(BPArm?.some($0)) } }; Picker("体位", selection: $position) { Text(BPText.localized("未设置", language: language)).tag(BodyPosition?.none); ForEach(BodyPosition.allCases, id: \.self) { Text($0.displayName(language: language)).tag(BodyPosition?.some($0)) } }; Picker("情绪", selection: $mood) { Text(BPText.localized("未设置", language: language)).tag(Mood?.none); ForEach(Mood.allCases, id: \.self) { Text($0.displayName(language: language)).tag(Mood?.some($0)) } }; Toggle("空腹测量", isOn: $isFasting).accessibilityIdentifier("fasting-toggle"); Toggle("测量前运动", isOn: $exerciseBefore); Toggle("已服药", isOn: $medicationTaken); TextField("备注（可选）", text: $note, axis: .vertical) }
