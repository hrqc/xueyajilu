...
- `CI_REPORT.md`：macOS CI 结果入口（首次运行前为未执行状态）
...
当前仓库运行环境为 Windows，无法执行 `xcodebuild`、iOS Simulator、HealthKit entitlement 或真实生物识别。已完成源码、规则、测试用例和文档生成；完整编译、UI 测试、覆盖率和性能门禁需在 macOS 上执行。Web 原型使用浏览器 localStorage，不应存放真实敏感健康数据；正式健康数据请使用 iOS App。
